...
class Settings(BaseSettings):
    ...
    query_workers: int = Field(default=2, ge=1, le=8)
    preserve_insertion_order: bool = False

    @property
    def resolved_database_path(self) -> Path:
        """Return the fixed v3 database location beneath ``MLD_DATA_ROOT``."""
        return self.data_root / "DryData.duckdb"

    @property
    def resolved_jobs_database_path(self) -> Path:
        """Return the fixed Job-control database beneath ``MLD_DATA_ROOT``."""
        return self.data_root / "Jobs.duckdb"

    @property
    def resolved_temp_directory(self) -> Path:
        """Return the fixed temporary directory beneath ``MLD_DATA_ROOT``."""
